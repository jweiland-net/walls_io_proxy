...
while getopts "a:b:d:i:s:t:p:xy:nhu" OPT; do
    case ${OPT} in
        a)
...
case ${TEST_SUITE} in
    ...
    composerUpdate)
        ...
        if [ ${TYPO3_VERSION} -eq 11 ]; then
            COMMAND=(composer require --no-ansi --no-interaction --no-progress --no-install typo3/cms-core:^11.5)
        fi
        if [ ${TYPO3_VERSION} -eq 12 ]; then
            COMMAND=(composer require --no-ansi --no-interaction --no-progress --no-install typo3/cms-core:^12.4)
        fi
